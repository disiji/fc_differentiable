...
def get_diagnostics_init_final(model, data, labels, tracker):
    ...
    expert_preds = np.array([0. if prop[0] <= .00015 else 1. for prop in
                             all_probs_f])  # change to just prop if using ModelTree instead of ModelTreeBoth
...
def write_model_diagnostics(model, dafi_model, input, tracker_train_m, tracker_eval_m, tracker_train_d, tracker_eval_d,
                            experiment_name):
    ...
    np.savetxt(save_prefix + 'tr_diagnostics_model.csv', np.array(list(tr_diagnostics_m.values())).reshape([1, -1]),
               header=header, delimiter=',', fmt=FMT_STR)
    np.savetxt(save_prefix + 'te_diagnostics_model.csv', np.array(list(te_diagnostics_m.values())).reshape([1, -1]),
               header=header, delimiter=',', fmt=FMT_STR)
    np.savetxt(save_prefix + 'tr_diagnostics_dafi.csv', np.array(list(tr_diagnostics_d.values())).reshape([1, -1]),
               header=header, delimiter=',', fmt=FMT_STR)
    np.savetxt(save_prefix + 'te_diagnostics_dafi.csv', np.array(list(te_diagnostics_d.values())).reshape([1, -1]),
               header=header, delimiter=',', fmt=FMT_STR)
...
def write_learned_logistic_weights(model, dafi_model, experiment_name):
    ...
    dafi_weight, dafi_bias = from_gpu_to_numpy(dafi_model.linear.weight.data[0]), from_gpu_to_numpy(
        dafi_model.linear.bias)
...
def run_write_full_output_for_CV(model, dafi_model, input, trackers_dict, hparams, model_checkpoint_dict,
    ...
    plt.savefig('../output/%s/pos_and_neg_gates_iter_%d.png' % (
        hparams['experiment_name'], hparams['seven_epochs_for_gate_motion_plot'][-1]))
    plt.clf()
