...
# load from csv
def scatter_vs_dafi_feature(dataname, method_name, metric_name):
    ...
    figname_train = '../fig/%s/%s_%s_train.png' % (dataname, method_name, metric_name)
    fig = plt.figure(figsize=(3, 3))
    ax = fig.add_subplot(1, 1, 1)
    ax.scatter(df['train_%s' % metric_name], df['train_%s_dafi' % metric_name], s=5)

    ax.set_xlabel('Model')
    ax.set_ylabel('Dafi features')
    ...
    figname_test = '../fig/%s/%s_%s_test.png' % (dataname, method_name, metric_name)
    fig = plt.figure(figsize=(3, 3))
    ax = fig.add_subplot(1, 1, 1)
...
def scatter_methods(dataname, method_name_1, method_name_2, metric_name):
    # todo: need to make df of differnet methods to plot of same length
    """
...
if __name__ == '__main__':
    ...
    for method_name in model_dict:
        for metric_name in metric_dict:
            scatter_vs_dafi_feature(dataname, method_name, metric_name)
