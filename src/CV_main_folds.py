...
def run_single_panel(hparams, random_state_start=0, model_checkpoint=True):
    ...
    for random_state in range(random_state_start, hparams['n_run']):
        ...
        seed_experiment_name = default_exp_name + '_seed%d' %random_state
        #hparams['experiment_name'] = default_exp_name + '_seed%d' %random_state
        np.random.seed(random_state)
        torch.manual_seed(random_state)
        folds = KFold(n_splits=hparams['n_folds_for_each_random_seed'], random_state=random_state)

        input_no_split = Cll8d1pInput(hparams)
        for fold_idx, (tr_idxs, te_idxs) in enumerate(folds.split(input_no_split.x_list)):
            fold_start_time = time.time()
            hparams['experiment_name'] = seed_experiment_name + '_fold%d' %fold_idx
            ...
            print('Fold %d took %d seconds' %(fold_idx, time.time() - fold_start_time))
...
if __name__ == '__main__':
    yaml_filename = '../configs/CV_runs_folds.yaml'
