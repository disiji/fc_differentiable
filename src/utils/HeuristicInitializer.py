...
class HeuristicInitializer:
    ...
    def find_best_gate(self, gate_idx):
        ...
        best_gate = None
        gate_data_axes = self.gate_data_axes_ids[gate_idx]
        heuristic_at_each_cell = -1. * np.ones(
            [self.num_gridcells_per_axis, self.num_gridcells_per_axis, self.NUM_CORNERS])
            ...
            for y in cell_locs_y:
                for corner_idx in range(self.NUM_CORNERS):
                    ...
                    if self.is_gate_acceptable(gate):
                        ...
                        heuristic_at_each_cell[int(x * self.num_gridcells_per_axis) - 1, int(
                            y * self.num_gridcells_per_axis) - 1, corner_idx] = np.around(heuristic_score, 3)
