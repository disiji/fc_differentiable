...
def baseline_plot_main(hparams_for_input, path_to_model_params):
    ...
    for num_clusters in hparams['num_clusters_grid']:
        ...
        save_files_for_flowsom(cll_1p_full_input, './temp_catted_data',
                               model_params['flowsom_params']['num_to_subsample'], num_clusters)
        ...
        x_eval_for_flowsom = [x[np.random.permutation(x.shape[0])][0:model_params['flowsom_params']['num_to_subsample']]
                              for x in x_eval]
    ...
    for l1_penalty in hparams['l1_logreg_penalty_grid']:
        ...
        for num_clusters in hparams['num_clusters_grid']:
            ...
            for fold_idx, (tr_idxs, te_idxs) in enumerate(folds.split(input_no_split.x_list)):
                ...
                save_files_for_flowsom(cll_1p_full_input, './temp_catted_data',
                                       model_params['flowsom_params']['num_to_subsample'], fold_idx)
                ...
                x_eval = [x.cpu().detach().numpy() for x in cll_1p_full_input.x_eval]
                x_eval_for_flowsom = [
                    x[np.random.permutation(x.shape[0])][0:model_params['flowsom_params']['num_to_subsample']] for x in
                    x_eval]
                y_eval = cll_1p_full_input.y_eval.cpu().detach().numpy()

                print('eval for flowsom:')
                preds_flowsom, eval_acc_flowsom = get_preds_and_eval_acc(model_flowsom, 'flowsom', x_eval_for_flowsom,
                                                                         y_eval)
                print('eval for kmeans:')
                preds_kmeans, eval_acc_kmeans = get_preds_and_eval_acc(model_kmeans, 'kmeans', x_eval, y_eval)
                avg_te_acc_fs_cur += eval_acc_flowsom
...
def runs_50_main(hparams_for_input, path_to_model_params):
    ...
    for s, random_seed in enumerate(seeds):
        ...
        save_files_for_flowsom(input, './temp_catted_data', model_params['flowsom_params']['num_to_subsample'],
                               random_seed)
        ...
        preds_kmeans2_aug, eval_acc_kmeans2_aug = get_preds_and_eval_acc(model_kmeans2_aug, 'kmeans2 aug', x_eval,
                                                                         y_eval)
        ...
        if s == 0:
            current_output = update_and_write_current_output(preds_and_acc_dict, y_eval, random_seed,
                                                             '../output/%s/' % hparams_for_input['experiment_name'])
        else:
            current_output = update_and_write_current_output(preds_and_acc_dict, y_eval, random_seed,
                                                             '../output/%s/' % hparams_for_input['experiment_name'],
